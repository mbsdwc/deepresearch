...
from hello_agents.tools import ToolRegistry, SearchTool
...
from prompts import (
    ...
    todo_planner_system_prompt, evidence_verifier_instructions,
)
from models import SummaryState, SummaryStateOutput, TodoItem
from services.planner import PlanningService
from services.reporter import ReportingService
from services.search import dispatch_search, prepare_research_context,EvidenceSearchTool
...
class DeepResearchAgent:
    ...
    def __init__(self, config: Configuration | None = None) -> None:
        ...
        self.evidence_search_tool = EvidenceSearchTool(self.config)
        registry.register_tool(self.evidence_search_tool)
        ...
        self.Verify_Agent = self._create_tool_aware_agent(
            name="证据核验专家",
            system_prompt=evidence_verifier_instructions.strip(),
        )
    ...
    def _init_llm(self) -> HelloAgentsLLM:
        ...
        print("DEBUG config:")
        print("provider =", self.config.llm_provider)
        print("model_id =", self.config.llm_model_id)
        print("local_llm =", self.config.local_llm)
        print("base_url =", self.config.llm_base_url)
    ...
    def _execute_task(
        ...
        *,
        ...
        {context}
        """
        logger.info("========== Evidence Check START ==========")
        logger.info("Task: %s", task.title)
        #logger.info("Evidence check input:\n%s", verify_prompt)

        verify_result = self.Verify_Agent.run(verify_prompt)

        logger.info("Evidence check result:\n%s", verify_result)
        logger.info("Evidence tool events: %s",self._tool_tracker.as_dicts())
        logger.info("========== Evidence Check END ==========")

        task.evidences = verify_result
        ...
        if emit_stream:
            ...
            summary_stream, summary_getter = self.summarizer.stream_task_summary(state, task, context, verify_result)
            try:
                for event in self._drain_tool_events(state, step=step):
                    yield event
            ...
            summary_text = self.summarizer.summarize_task(state, task, context, verify_result or "")
            self._drain_tool_events(state)
